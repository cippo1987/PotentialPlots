...
if dimensions == 3:
# 3D Case
 Potential_Array_Size=int(math.ceil(Lattice[2,2]/Bin_Width))
elif dimensions == 2:
# 2D Case
 Potential_Array_Size=int(math.ceil((max(Coordinates[:,2])-min(Coordinates[:,2]))//Bin_Width))
Potential_Average=numpy.zeros(shape=(Potential_Array_Size,3))

for x in range(0,Potential_Array_Size):
 Potential_Average[x,0]=x*Bin_Width
 for y in range(0,num_atoms):
...
Potential_Interp=interp1d(Potential_Average[:,0],Potential_Average[:,1],kind='cubic')
...
for x in range(0,Potential_Array_Size):
 print '%4.2f %10.6f' % (Potential_Average[x,0], Potential_Average[x,1])
 line = (Potential_Average[x,0], Potential_Average[x,1])
 l = str(line)
 f.write(l)
